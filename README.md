## Hi there 👋

# Hi 👋, I'm Soumyojit

🎓 **3rd Year B.Tech Student** at **Adamas University**  
💻 **Full Stack Java Developer | Machine Learning Enthusiast**  
🚀 Passionate about building scalable applications and intelligent systems

---

## 🔭 Currently Working On
- Full Stack Java projects (Spring Boot + MySQL)
- Machine Learning models using Python
- Data Structures & Algorithms (C / C++)
- Real-world problem solving & system design basics

## 🌱 Currently Learning
- Advanced Machine Learning techniques
- Backend optimization with Spring Boot
- REST APIs & Microservices
- Cloud & deployment fundamentals

## 👯 Looking to Collaborate On
- Full Stack Java applications
- Machine Learning & Data Science projects
- Open-source contributions
- Backend or database-driven systems

## 🤔 Looking For Help With
- Advanced ML models & deployment
- System design interviews
- Scalable backend architecture

## 💬 Ask Me About
- C, C++ & Python
- Machine Learning
- Full Stack Java Development
- MySQL & Database Design
- Git & GitHub

## 📫 How to Reach Me
- 📧 Email: **soumyojitbanerjee14@gmail.com**
- 🐙 GitHub: [https://github.com/Soumyojit-code](https://github.com/Soumyojit-code)


## 😄 Pronouns
He / Him

## ⚡ Fun Fact
I love combining **Machine Learning** with **Full Stack Development** to build smart applications 🤖🚀

---

## 🛠️ Tech Stack

### 👨‍💻 Programming Languages
- C  
- C++  
- Python  
- Java  

### 🧠 Machine Learning
- Supervised & Unsupervised Learning
- NumPy, Pandas, Matplotlib
- Scikit-learn

### 🌐 Full Stack Java
- Java, Spring Boot
- REST APIs
- HTML, CSS, JavaScript
- MySQL

### 🗄️ Databases
- MySQL

### 🔧 Tools & Platforms
- Git & GitHub
- Jupyter Notebook
- VS Code

---

⭐ *Feel free to explore my repositories and connect with me!*
